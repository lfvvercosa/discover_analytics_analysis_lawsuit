...
paths_metrics = [
    base_path + 'ARI_1step_ngram_kms_ics.csv',
...
for i in range(len(paths_metrics)):
    ...
    bp_act = plt.boxplot(df_act, 
                      positions=np.array(range(len(df_act.columns)))*3.0-0.8, 
                      sym='', 
                      widths=0.6)
    ...
    set_box_color(bp_act, '#D7191C') # colors are from http://colorbrewer2.org/
    ...
    plt.plot([], c='#D7191C', label='ActiTraC')
    ...
    path_output = out_path + 'Comp3_lev' + metric_name + '.png'
