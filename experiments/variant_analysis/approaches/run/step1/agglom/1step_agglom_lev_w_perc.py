...
def calc_overall_fitness_perc(df_labels,
    ...
    for c in bigger_clusters:
        ...
        fit = markov_fitness.get_fitness_mean(p=p)
        weight_fit += fit * c[1]
...
if __name__ == '__main__':
    ...
    local_file = 'experiments/variant_analysis/exp4/results/1step_agglom_lev_w_perc.txt'
    filename = 'variant_analysis/exp4/results/1step_agglom_lev_w_perc.txt'
    content = ""

    params_agglom = {}
    params_agglom['AWS_bucket'] = bucket
    params_agglom['AWS_filename'] = \
        'variant_analysis/exp4/results/1step_agglom_lev_w_perc.txt'
    ...
    total = len(minimum_pts) * len(weight_cluster_number) * \
            len(mean_perc) * len(k_markov) * len(weight_parallel) * \
