...
def obtain_breadscrum(df, df_code_subj, col_old, col_new):
    df_bs = df_code_subj.rename(columns={col_old:col_new})\
              [[col_new,'breadscrum']]

    df = df.merge(df_bs, on=col_new, how='left')
...
def get_subject_level(breadscrum, level):
    if type(breadscrum) == str:
        ...
        if len(breadscrum) > level:
            ...
            return int(breadscrum[len(breadscrum)-1])
    else:
        return -1
    
    
def create_1_gram_subjects(df, 
                           df_code_subj, 
                           min_perc, 
                           max_perc, 
                           level):
    ...
    subject_col = 'case:lawsuit:subjects'

    df_gram = df[[id_col, subject_col]].drop_duplicates(subset=id_col)
    df_gram = df_gram[[id_col, subject_col]].explode(subject_col)
    df_gram = obtain_breadscrum(df_gram, 
                                df_code_subj, 
                                col_old='assuntoCodigoNacional',
                                col_new=subject_col)
    
    df_gram[subject_col] = df_gram.apply(lambda df: get_subject_level(
                                                df['breadscrum'],
                                                level), axis=1
                                   )
    
    df_gram = df_gram[df_gram[subject_col] != -1]
    
    df_gram = my_cluster.create_n_gram(df_gram, 
                                       id_col, 
                                       subject_col,
                                       1)
    ...
    if not df_gram.empty:
        ...
        for c in cols:
            new_names[c] = 'ASSU_' + str(c)

        df_gram = df_gram.rename(columns=new_names)
...
def create_n_gram_movements(df, min_perc, max_perc, n):
    id_col = 'case:concept:name'
...
def get_mesorregiao(uf, meso):
    if meso == None or uf == None:
        return None
    else:
        return str(uf) + '_' + str(meso)


def get_ibge_features(df, df_ibge):
    df_work = df_ibge[['Código Município Completo',
                       'UF',
                       'Mesorregião Geográfica',
                       'Nome_UF']]
    df_work = df_work.rename(columns={'Código Município Completo':
                                      'case:court:county'})
    
    df_work['Mesorregiao'] = df_work.apply(lambda df: get_mesorregiao(
                                            df['Nome_UF'],
                                            df['Mesorregião Geográfica'],
                                            ), axis=1)

    df_work = df_work[['case:court:county','Mesorregiao','Nome_UF']]

    df_feat = df[['case:court:county']].drop_duplicates()
    df_feat = df_feat.merge(df_work, on='case:court:county',how='left')
...
def get_clus_actitrac_feats(log_path):
    ...
    df_clus = actitrac.run(n_clusters=15,
                                    is_greedy=True,
                                    dist_greed=0.1,
                                    target_fit=0.5,
                                    min_clus_size=0.05,
