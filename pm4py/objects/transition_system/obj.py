...
class TransitionSystem(object):
    class State(object):
        def __init__(self, name, incoming=None, outgoing=None, data=None):
            ...
            self.__data = {constants.INGOING_EVENTS: [], constants.OUTGOING_EVENTS: [],
                           constants.MAP_INCOMING:{}, constants.MAP_OUTGOING:{},
                           constants.ALL_INCOMING:[]} if data is None else data
        ...
        def outgoing_data(self, trans):
            self.outgoing.add(trans)
            self.data[constants.MAP_OUTGOING][trans.name] = trans

        def incoming_data(self, trans):
            self.incoming.add(trans)
            self.data[constants.MAP_INCOMING][trans.name] = trans
    ...
    class Transition(object):
        ...
        # def __eq__(self, obj):
        #     return obj.name == self.name and \
        #            obj.from_state == self.from_state and \
        #            obj.to_state == self.to_state
