...
from tests.unit_tests.test_random_filter import getNumberLogVariants
...
def apply_log(log, dfg, sa, ea, parameters=None):
    ...
    # print('computing partial alignments...')
    total_var = getNumberLogVariants(log)
...
def __apply_list_activities(trace, dfg, sa, ea, parameters=None):
    ...
    if my_debug:
        my_file = open("temp/trace_partial_alignment.txt", "w+")
        my_file.write('TRACE: ' + str(trace) + '\n\n')
    ...
    while not len(open_set) == 0:
        ...
        if my_debug:
            my_file.write(str(__return_alignment(curr, trace, num_closed)))
            my_file.write('\n\n')
