...
def __transform_trace_to_mem_efficient_structure(trace, model_struct, parameters=None):
    ...
    elif type(trace_cost_function) is dict:
        trace_cost_function = get_trace_cost_function(trace, parameters)
...
def get_trace_cost_function(trace, parameters):
    trace_cost = {}
    activity_key = exec_utils.get_param_value(Parameters.ACTIVITY_KEY, 
                                              parameters, DEFAULT_NAME_KEY)
    cost_func = exec_utils.get_param_value(Parameters.PARAM_TRACE_COST_FUNCTION, 
                                           parameters, 
                                           None)

    for i in range(len(trace)):
        trace_cost[i] = cost_func[trace[i][activity_key]]
